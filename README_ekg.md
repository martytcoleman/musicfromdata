# EKG → Music: Sonification of 12-Lead Cardiac Recordings

This script converts real 12-lead ECG waveform data into a multi-layered audio piece. The idea is to map each lead group to a distinct instrument and let the actual physiology — heart rate, QRS duration, QT interval, cardiac axis — drive the musical parameters instead of inventing them.
...
- **Note duration** = derived from QRS width (wider QRS → longer notes)
- **Reverb (CC 91)** = derived from QT interval (longer QT → more reverb/tail)
...
Four different recordings are included, each with its own pentatonic/modal scale and tempo curve:

- `harp` — recorded while playing harp; sinus tachycardia ~95–102 bpm
- `binaural` — binaural beat listening session; HR decelerates 90 → 71 bpm
- `raga` — raga listening; parasympathetic deceleration 102 → 80 bpm
- `guitar` — guitar playing; sympathetic arousal with variable HR 90–118 bpm
...
The script will print progress as it processes each lead group and calls FluidSynth to render audio. Output is a single mixed WAV file written to `public/`:
